...
1. Конструктор принимает `random_state` и все параметры, объявленные для
   модели в JSON. Если в сигнатуре есть `n_components`, UI-раннер передает
   истинное число компонент датасета (или число наблюдений при инициализации
   по точкам данных). Smooth EM определяет число компонент сам и не принимает
   этот аргумент.
...
Для сравнения при неизвестном числе компонент и тяжёлых хвостах используйте:

```bash
python run.py --config-name unknown_k_student_t_showcase
```

Здесь обычный EM исключён, а Bayesian EM, GMMoments и Smooth EM стартуют с
верхней границы `K_max=K0=24` при истинном `K*=3`. Компоненты имеют распределение
Student-t с `df=3`, одинаковую ковариацию `I` и равные веса. Сетка содержит
60 комбинаций `d`, `n` и разделения. Тяжёлые хвосты проверяют восстановление
латентных популяций при отклонении от гауссовой модели: методы с 24 свободными
компонентами склонны описывать хвосты дополнительными кластерами, тогда как
Smooth EM отсекает лишние компоненты. Метрики параметров вычисляются относительно
гауссовой смеси с теми же центрами, весами и ковариациями.
