...
def fit_dimension_free_one_s_gmm(
    ...
    assert np.all(sigmas > 0), "sigmas must be positive"
    ...
    def q_single(m, sigma):
        ...
        return np.exp(-0.5 * norm2 / t) * (s2 / t) * h
    ...
    def build_Q(means_, sigmas_):
        ...
        return np.exp(-0.5 * norm2 / t[None, :]) * (s2 / t)[None, :] * h
    ...
    for _ in iterator:
        ...
        for _ in range(geom_sweeps):
            ...
            for k in range(K):
                ...
                m_new = res.x[:d]
                sigma_new = np.sqrt(np.exp(res.x[-1]))
...
def fit_dimension_free_one_s_gmm_joint(
    ...
    def unpack_geometry(theta):
        ...
        variances_ = np.exp(component_params[:, -1])
    ...
    for _ in iterator:
        ...
        for _ in range(geom_sweeps):
            ...
            def evaluate_model_and_jacobian(theta):
                ...
                h = np.einsum("jkd,jd->jk", r, U)
                norm2 = np.einsum("jkd,jkd->jk", r, r)
                scale = np.exp(-0.5 * norm2 / t[None, :]) * (s2 / t)[None, :]
                Q_ = scale * h
